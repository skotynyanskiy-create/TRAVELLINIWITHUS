...
import { saveContactLead } from '../services/firebaseService';
...
export default function Contatti() {
  ...
  const topicGuidance: Record<string, { hint: string; placeholder: string }> = {
    ...
    article: {
      ...
      placeholder: 'Scrivici a quale articolo, guida o risorsa ti riferisci e cosa ti serve sapere...',
    },
    other: {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSubmitError(`Non siamo riusciti a inviare il messaggio. Puoi scriverci direttamente a ${CONTACTS.email}.`);
    } finally {
        ...
        <div className="mx-auto mt-8 mb-20 max-w-3xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="mb-6 flex items-center justify-center gap-4">
              ...
              <span className="text-sm font-semibold uppercase tracking-widest text-[var(--color-accent)]">{pageContent.heroEyebrow}</span>
              <div className="h-[1px] w-12 bg-[var(--color-accent)]"></div>
            </div>
          ...
          <div className="space-y-8 lg:col-span-2">
            ...
            <div className="group flex items-start gap-4 rounded-[var(--radius-xl)] border border-black/5 bg-[var(--color-sand)] p-6 transition-all duration-500 hover:shadow-[var(--shadow-premium)]">
              ...
              <div>
                ...
                <p className="mb-2 text-sm font-normal text-black/70">{pageContent.emailCardDescription}</p>
                <a
            ...
            >
              <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-[var(--radius-xl)] bg-white shadow-sm transition-transform duration-500 group-hover:scale-110`} style={{ color: SOCIAL_COLORS.whatsapp }}>
                <MessageCircle size={20} />
              </div>
              <div>
                <h4 className="mb-1 font-serif text-xl">{pageContent.whatsappCardTitle}</h4>
                <p className="mb-2 text-sm font-normal text-black/70">{pageContent.whatsappCardDescription}</p>
                <span className="text-sm font-medium transition-colors group-hover:text-[#25D366]">{CONTACTS.whatsappDisplay}</span>
              </div>
            </a>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                <Instagram size={28} className="text-black/60 transition-colors duration-500 group-hover:scale-110 group-hover:text-[#E1306C]" />
              ...
              <a
                ...
                className="group flex flex-col items-center justify-center gap-3 rounded-[var(--radius-xl)] border border-black/5 bg-[var(--color-sand)] p-6 transition-all duration-500 hover:shadow-[var(--shadow-premium)]"
              >
                <svg className="h-7 w-7 text-black/60 transition-colors duration-500 group-hover:scale-110 group-hover:text-black" viewBox="0 0 24 24" fill="currentColor">
              ...
              {!isSubmitted ? (
                  ...
                  <form className="space-y-8" onSubmit={handleSubmit} noValidate>
                    <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
                      <div className="space-y-2">
                        <label htmlFor="name" className="text-[10px] font-bold uppercase tracking-widest text-black/50">
                          Nome / Azienda *
                        </label>
                        <input
                          type="text"
                          ...
                          className={`w-full border-b bg-transparent py-3 transition-colors focus:outline-none ${
                            errors.name ? 'border-red-500 focus:border-red-500' : 'border-black/10 focus:border-[var(--color-accent)]'
                          }`}
                      ...
                      <div className="space-y-2">
                        <label htmlFor="email" className="text-[10px] font-bold uppercase tracking-widest text-black/50">
                          Email *
                        </label>
                        <input
                          type="email"
                          ...
                          onChange={handleChange}
                          className={`w-full border-b bg-transparent py-3 transition-colors focus:outline-none ${
                            errors.email ? 'border-red-500 focus:border-red-500' : 'border-black/10 focus:border-[var(--color-accent)]'
                          }`}
                    ...
                    <div className="space-y-2">
                      <label htmlFor="topic" className="text-[10px] font-bold uppercase tracking-widest text-black/50">
                        Motivo del contatto *
                      </label>
                      <select
                        id="topic"
                        ...
                        className={`w-full appearance-none rounded-none border-b bg-transparent py-3 text-black transition-colors focus:outline-none ${
                          errors.topic ? 'border-red-500 focus:border-red-500' : 'border-black/10 focus:border-[var(--color-accent)]'
                        }`}
                      >
                        <option value="" disabled>
                          Seleziona un&apos;opzione
                        </option>
                        <option value="collab">Proposta di collaborazione / sponsorizzazione</option>
                    ...
                    <div className="space-y-2">
                      <label htmlFor="message" className="text-[10px] font-bold uppercase tracking-widest text-black/50">
                        Messaggio *
                      </label>
                      <p className="text-xs font-light leading-relaxed text-black/45">{activeGuidance.hint}</p>
                      <textarea
                        id="message"
                        rows={5}
                        ...
                        className={`w-full resize-none border-b bg-transparent py-3 transition-colors focus:outline-none ${
                          errors.message ? 'border-red-500 focus:border-red-500' : 'border-black/10 focus:border-[var(--color-accent)]'
                        }`}
                    ...
                    <div className="pt-6">
                      <p className="mb-8 text-xs font-light leading-relaxed text-black/40">
                        ...
                        <Link to="/privacy" className="underline underline-offset-2 transition-colors hover:text-black">
                          Privacy Policy
                        </Link>{' '}
                        per il trattamento dei dati personali. Rispondiamo di solito entro 24-48 ore lavorative, quando
                        il contatto è chiaro e completo.
                      </p>
                      {submitError && <p className="mb-6 text-sm text-red-600">{submitError}</p>}
                      <Button type="submit" variant="primary" size="lg" className="w-full px-12 md:w-auto">
                        {isSubmitting ? (
                          <>
                            Invio in corso
                ...
                >
                  ...
                  <p className="mx-auto mb-10 max-w-md text-lg font-normal leading-relaxed text-black/70">
                    Grazie per averci contattato. Abbiamo ricevuto il tuo messaggio e ti risponderemo appena possibile,
                    in genere entro 24-48 ore lavorative.
                  </p>
                  <button
                    onClick={() => {

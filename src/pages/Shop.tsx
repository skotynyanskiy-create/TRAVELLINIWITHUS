...
const reasons = [
  {
    ...
    text: 'Nessun template generico. Ogni pagina è testata, ogni consiglio è vero.',
  },
  {
    icon: <Map className="text-accent" size={22} />,
    title: 'Nati dall\'esperienza',
    text: 'Itinerari e guide scritti dopo aver vissuto quei luoghi in prima persona — non da ricerche online.',
  },
  {
    icon: <Shield className="text-accent" size={22} />,
    title: 'Garanzia di qualità',
    text: 'Se li usiamo noi e funzionano, li mettiamo qui. Altrimenti no.',
  },
];

export default function ShopWrapper() {
  return (
    <ErrorBoundary fallback={<div className="text-center py-20 text-red-500">Impossibile caricare i prodotti</div>}>
      <Shop />
    </ErrorBoundary>
  );
}
              ...
              {paymentStatus === 'success' ? (
                <>
                  ...
                  <p className="text-zinc-600 mb-8">
                    Grazie per il tuo acquisto. Riceverai a breve i dettagli dell'ordine e le istruzioni per accedere
                ...
                <>
                  ...
                  <p className="text-zinc-600 mb-8">
                    Il processo di pagamento è stato interrotto. Nessun addebito è stato effettuato.
                  </p>
                </>
              )}
              <button
      ...
      <Section className="pt-8">
        ...
        <div className="relative text-center max-w-5xl mx-auto mb-24 mt-8 p-12 md:p-20 bg-zinc-50/50 rounded-3xl border border-accent/10 shadow-sm overflow-hidden">
          ...
          <p className="text-lg text-zinc-600 font-light leading-relaxed mt-10 max-w-2xl mx-auto relative z-10">
            Queste guide e risorse nascono da {BRAND_STATS.yearsOfTravel} anni di viaggi reali. Non sono template generici. Ogni pagina è testata sul campo, ogni consiglio è autentico. Quando li usiamo noi, funzionano.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {reasons.map((item) => (
            <div key={item.title} className="bg-white p-8 rounded-4xl border border-black/5 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16 bg-white/50 backdrop-blur-md p-6 rounded-4xl border border-black/5">
          <div className="flex items-center gap-6 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/30 shrink-0 ml-2">Filtra</span>
            {categories.map((cat) => (
              <button
                key={cat}
          ...
          <div className="flex items-center gap-4 w-full md:w-auto shrink-0 pr-2">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-black/30 shrink-0">Ordina</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 md:gap-12">
              {filteredAndSortedProducts.length > 0 ? (
                filteredAndSortedProducts.map((product, idx) => {
                  ...
                  return (
                    <div key={product.id} className={`${isLarge ? 'md:col-span-2 lg:col-span-8' : 'lg:col-span-4'}`}>
                      <ProductCard
                        id={product.id}
                        name={product.name}
                        slug={product.slug}
                        ...
                        isBestseller={(product as Product & { isBestseller?: boolean }).isBestseller || isLarge}
                        badgeLabel={usingShopDemo && product.slug === DEMO_PRODUCT.slug ? 'In arrivo' : undefined}
                      />
                    </div>
                  );
                })
                ...
                <div className="col-span-full py-20 text-center">
                  ...
                  <p className="text-base font-normal text-black/65">Prova a cambiare categoria o a rimuovere i filtri attivi.</p>
                </div>
              ) : null}
            </div>
          )}
        ...
        {!isLoading && products.length === 0 && (
          <div className="mt-12 rounded-[2rem] border border-black/5 bg-white p-10 text-center shadow-sm">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-base font-normal leading-relaxed text-black/70">
              Questa sezione è già pronta per ospitare i tuoi contenuti premium reali. Quando inserirai guide,
              itinerari, planner o raccolte curate, appariranno qui con la stessa struttura.
            </p>
          </div>
        )}

        <div className="mt-40 relative rounded-[3.5rem] overflow-hidden bg-ink text-white p-12 md:p-24 shadow-2xl">
          <div className="absolute inset-0 opacity-20">
             {/* TODO(@travelliniwithus): PLACEHOLDER — servono foto sfondo sezione shop dark */}
             <img
               src="https://images.unsplash.com/photo-1476514525535-07fb3b4ae5f1?q=80&w=2400&auto=format&fit=crop"
               className="w-full h-full object-cover"
               alt="Sfondo sezione risorse digitali di viaggio" 
             />
             <div className="absolute inset-0 z-10 bg-linear-to-b from-transparent via-ink/80 to-ink"></div>
          </div>
          ...
          <div className="relative z-20 h-full flex flex-col justify-center px-6 md:px-12 lg:px-24">
            ...
            >
              ...
              <p className="text-xl text-white/70 font-light leading-relaxed mb-12 max-w-xl">
                Non offriamo template statici, ma distillati di esperienza. Ogni guida e ogni planner è nato in viaggio, testato sul campo, pensato per aiutarti a partire davvero preparato.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div className="p-8 rounded-3xl bg-[#1C1C1C] border border-white/8">
                  ...
                  <p className="text-sm text-white/50 font-light leading-relaxed">
                    Ottimizzati per lo smartphone. Consulta i tuoi itinerari mentre sei in treno, in aereo o cammini per la città.
                  </p>
                </div>
                <div className="p-8 rounded-3xl bg-[#1C1C1C] border border-white/8 text-white">
                  ...
                  <p className="text-sm text-white/50 font-light leading-relaxed">
                    Cura editoriale da magazine di viaggio e praticità da guida tecnica. Niente rumore, solo valore.
                  </p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="aspect-4/5 rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-zinc-900">
                {/* TODO(@travelliniwithus): PLACEHOLDER — servono foto sezione review prodotto */}
                <img
                  src="https://images.unsplash.com/photo-1488646953014-85cb44e25828?q=80&w=1200&auto=format&fit=crop"
                  alt="Sfondo sezione recensioni prodotto"
                  className="w-full h-full object-cover opacity-60"
                  referrerPolicy="no-referrer"
                />
              </div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] bg-white text-ink p-10 rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] border border-black/5 text-center">
                 <div className="flex items-center justify-center gap-2 mb-6 text-accent">
                   {[1, 2, 3, 4, 5].map((i) => (
                     <Star key={i} size={16} className="fill-accent" />
                   ))}
                 </div>
                 <p className="italic font-serif text-2xl mb-6 text-ink leading-relaxed">
                   "Ogni guida che trovi qui è stata testata nei nostri viaggi. Se non funziona per noi, non finisce in vetrina."
                 </p>
                 <div className="flex flex-col items-center">
                    <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-black/30">L'impegno di</span>
                    <span className="font-serif text-lg mt-1 text-accent">Travelliniwithus</span>
                 </div>
              </div>
            </motion.div>
          </div>
        ...
        <div className="mt-32 bg-white rounded-3xl p-12 md:p-20 text-center border border-black/5">
          ...
          <p className="text-lg text-black/70 font-light max-w-2xl mx-auto mb-10">
            Scrivici direttamente — rispondiamo a ogni messaggio. Se hai una domanda specifica su un itinerario o vuoi capire quale contenuto fa per te, siamo qui.
          </p>

...
const groupVisuals: Record<string, string> = {
  Italia: 'https://images.unsplash.com/photo-1523906834658-6e24ef2386f9?q=80&w=1200&auto=format&fit=crop',
  Europa: 'https://images.unsplash.com/photo-1467269204594-9661b134dd2b?q=80&w=1200&auto=format&fit=crop',
  Asia: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?q=80&w=1200&auto=format&fit=crop',
  Americhe: 'https://images.unsplash.com/photo-1483728642387-6c3bdd6c93e5?q=80&w=1200&auto=format&fit=crop',
  Africa: 'https://images.unsplash.com/photo-1516026672322-bc52d61a55d5?q=80&w=1200&auto=format&fit=crop',
  Oceania: 'https://images.unsplash.com/photo-1523482580672-f109ba8cb9be?q=80&w=1200&auto=format&fit=crop',
};
...
export default function Destinazioni() {
  ...
  const filteredItems = useMemo(() => {
    return archiveItems.filter((item) => {
      ...
      return matchGroup && matchRegion && matchCity && matchExperience;
    });
  ...
  const activeGroupImage = groupVisuals[selectedGroup === 'Tutti' ? 'Italia' : selectedGroup] || groupVisuals.Italia;
  ...
  return (
    <PageLayout>
      ...
      {/* --- IMMERSIVE HERO --- */}
      <section ref={heroRef} className="relative flex min-h-[60vh] items-end overflow-hidden bg-ink pb-16 pt-32 md:min-h-[70vh] md:pt-40">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-6 md:px-12">
          <div className="mb-6">
            <Breadcrumbs items={breadcrumbItems} className="[&_a]:text-white/60 [&_span]:text-white/40 [&_svg]:text-white/30" />
          </div>
          <span className="mb-4 block font-script text-xl text-[var(--color-accent)] md:text-2xl">Archivio principale</span>
          <h1 className="mb-6 text-5xl font-serif font-medium leading-none text-white md:text-7xl lg:text-8xl">
            Destinazioni, regioni<br />
          ...
          <p className="max-w-xl text-lg font-normal leading-relaxed text-white/85">
            Tutto quello che abbiamo visitato, mangiato e vissuto — organizzato per farti trovare subito il posto giusto.
          </p>
          ...
          >
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-white/60">contenuti trovati</span>
          </motion.div>
        </div>
      </section>
      ...
      <Section>
        ...
        <div className="mb-16 hidden md:block">
          <div className="relative overflow-hidden rounded-[2rem] border border-black/5 shadow-sm">
            <div className="absolute left-0 top-0 z-10 rounded-br-2xl bg-white/90 px-6 py-3 backdrop-blur-md">
              <span className="font-script text-lg text-[var(--color-accent)]">Esplora la mappa</span>
            </div>
            <InteractiveMap />
          </div>
        </div>
        ...
        <div className="mb-16 block md:hidden">
          ...
          >
            <div>
              <span className="block font-script text-lg text-[var(--color-accent)]">Mappa interattiva</span>
              <span className="text-sm font-light text-black/60">Scopri tutti i contenuti sulla mappa</span>
            </div>
        ...
        <div className="mb-12 flex w-full flex-col gap-6 rounded-[var(--radius-xl)] border border-[var(--color-ink)]/5 bg-[var(--color-surface)] p-6 shadow-sm">
          ...
          {(selectedGroup !== 'Tutti' || selectedRegion !== 'Tutti' || selectedCity !== 'Tutti' || selectedExperience !== 'Tutti') && (
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-black/40 mr-2">Filtri attivi:</span>
              {selectedGroup !== 'Tutti' && (
              ...
              {selectedExperience !== 'Tutti' && (
                <button onClick={() => updateSearch({ type: null })} className="inline-flex items-center gap-1.5 rounded-full bg-[var(--color-ink)] px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-white">
          ...
          {availableExperiences.length > 1 && (
            <div className="flex flex-col gap-4">
              <span className="text-xs font-bold uppercase tracking-widest text-black/50">Tipo esperienza</span>
              <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
                {availableExperiences.map((experience) => {
                  const visual = experience !== 'Tutti' ? getExperienceVisual(experience) : null;
                  const Icon = visual?.icon;
                  return (
                    <button
                      key={experience}
                      onClick={() => updateSearch({ type: experience === 'Tutti' ? null : slugifyExperienceType(experience) })}
                      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-widest transition-colors ${
                        selectedExperience === experience ? 'bg-black text-white' : 'bg-white text-black/60 hover:bg-black/5'
                      }`}
                    >
                      {Icon && <Icon size={13} />}
                      {experience}
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="absolute bottom-8 left-8 right-8 transition-all duration-500 md:group-hover:translate-y-4 md:group-hover:opacity-0">
                          ...
                          <h3 className="mb-4 text-2xl font-serif leading-tight text-white md:text-3xl">{item.title}</h3>

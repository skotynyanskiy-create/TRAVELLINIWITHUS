...
import { fetchStats, updateStats, SiteStats, seedTestData, logActivity, exportToCSV } from '../../services/firebaseService';
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'pages' | 'articles' | 'products' | 'stats' | 'analytics' | 'media' | 'orders' | 'users' | 'coupons' | 'logs'>('overview');
  ...
  const totalRevenue = orders.reduce((acc, order) => acc + (order.status === 'completed' ? order.total : 0), 0);
  const pendingOrders = orders.filter(o => o.status === 'pending').length;

  const filteredArticles = articles.filter(a => a.title.toLowerCase().includes(searchQuery.toLowerCase()) || a.slug.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <Section className="pt-32 pb-24 min-h-screen">
        {isPreviewMode && (
          <div className="mb-8 rounded-2xl border border-amber-200 bg-amber-50 px-6 py-4 text-sm text-amber-800">
            Modalita anteprima admin locale attiva. Puoi esplorare struttura e editor, ma le azioni che scrivono dati reali restano disattivate finche non completi il login Firebase.
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-zinc-100 overflow-hidden">
          <div className="p-6 border-b border-zinc-100 bg-zinc-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 className="text-lg font-semibold">
              {activeTab === 'overview' ? 'Panoramica Attività' : activeTab === 'pages' ? 'Pagine e contenuti istituzionali' : activeTab === 'articles' ? 'Articoli Pubblicati' : activeTab === 'products' ? 'Prodotti nel Negozio' : activeTab === 'orders' ? 'Ordini Ricevuti' : activeTab === 'users' ? 'Utenti Registrati' : activeTab === 'media' ? 'Libreria Media' : activeTab === 'analytics' ? 'Analisi Vendite' : 'Statistiche Collaborazioni'}
            </h2>
          ...
          {loading ? (
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={data}>
                        ...
                        />
                        <Area type="monotone" dataKey="sales" stroke="#9A7B4F" strokeWidth={3} fillOpacity={1} fill="url(#colorSales)" />
                      </AreaChart>
                    </ResponsiveContainer>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-4">
                    {leads.slice(0, 5).map(lead => (
                      <div key={lead.id} className="flex items-center justify-between p-4 rounded-xl bg-zinc-50 border border-zinc-100">
                        <div>
                        ...
                        <div className="text-right">
                          <p className="text-xs text-zinc-400">
                            {(lead.createdAt as TimestampLike)?.toDate ? (lead.createdAt as TimestampLike).toDate().toLocaleDateString() : 'Recent'}
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                {siteContentDefinitions.map((page) => (
                  <div key={page.id} className="rounded-2xl border border-zinc-100 bg-zinc-50 p-6 shadow-sm">
                    ...
                    <p className="mb-6 min-h-[72px] text-sm leading-relaxed text-zinc-600">{page.description}</p>
            ...
            <div className="p-8 max-w-2xl">
              <div className="space-y-6">
                <div>
                  <label htmlFor="igFollowers" className="block text-sm font-medium text-zinc-700 mb-2">Follower Instagram</label>
                  <input 
                    id="igFollowers"
                    type="text" 
                ...
                <div>
                  <label htmlFor="monthlyReach" className="block text-sm font-medium text-zinc-700 mb-2">Copertura Mensile (Reach)</label>
                  <input 
                    id="monthlyReach"
                    type="text" 
                ...
                <div>
                  <label htmlFor="uniqueUsers" className="block text-sm font-medium text-zinc-700 mb-2">Audience totale dichiarata</label>
                  <input 
                    id="uniqueUsers"
                    type="text" 
                ...
                <div>
                  <label htmlFor="engagementRate" className="block text-sm font-medium text-zinc-700 mb-2">Tasso di Engagement</label>
                  <input 
                    id="engagementRate"
                    type="text" 
                ...
                >
                  {savingStats ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}
                  Salva Statistiche
                </button>
              </div>
            </div>
            ...
            filteredArticles.length === 0 ? (
              <div className="p-12 text-center text-zinc-500">Nessun articolo trovato. Inizia a scrivere!</div>
            ) : (
              <div className="divide-y divide-zinc-100">
                {filteredArticles.map(article => (
                  <div key={article.id} className="p-6 flex items-center justify-between hover:bg-zinc-50 transition-colors">
                    <div>
                      <h3 className="font-medium text-lg">{article.title}</h3>
                      <p className="text-sm text-zinc-500">/{article.slug}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${article.published ? 'bg-[var(--color-accent-soft)] text-[var(--color-accent-text)]' : 'bg-yellow-100 text-yellow-700'}`}>
                        {article.published ? 'Pubblicato' : 'Bozza'}
                      </span>
                      <Link to={`/admin/editor/${article.id}`} className="p-2 text-zinc-400 hover:text-[var(--color-accent)] transition-colors">
                        <Edit size={18} />
                      </Link>
                      <button onClick={() => handleDeleteArticle(article.id)} className="p-2 text-zinc-400 hover:text-red-500 transition-colors">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
            ...
            filteredProducts.length === 0 ? (
              <div className="p-12 text-center text-zinc-500">Nessun prodotto trovato. Aggiungi il tuo primo prodotto!</div>
            ) : (
              <div className="divide-y divide-zinc-100">
                {filteredProducts.map(product => (
                  <div key={product.id} className="p-6 flex items-center justify-between hover:bg-zinc-50 transition-colors">
                    <div>
                      <h3 className="font-medium text-lg">{product.name}</h3>
                      <p className="text-sm text-zinc-500">/{product.slug} • €{product.price.toFixed(2)}</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700 uppercase tracking-wider">
                        {product.category}
                      </span>
                      <Link to={`/admin/product-editor/${product.id}`} className="p-2 text-zinc-400 hover:text-[var(--color-accent)] transition-colors">
                        <Edit size={18} />
                      </Link>
                      <button onClick={() => handleDeleteProduct(product.id)} className="p-2 text-zinc-400 hover:text-red-500 transition-colors">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>

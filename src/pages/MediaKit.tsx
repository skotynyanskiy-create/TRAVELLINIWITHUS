...
import { saveMediaKitLead } from '../services/firebaseService';
...
const includes = [
  {
    ...
    description: 'Posizionamento, tono, pubblico di riferimento e modo in cui Travelliniwithus lavora e comunica.',
    icon: BriefcaseBusiness,
  },
  {
    title: 'Metriche e audience',
    description: 'Dati aggiornati e panoramica utile per valutare compatibilità, potenziale e taglio della collaborazione.',
    icon: BarChart3,
  },
  {
    title: 'Formati e deliverable',
    description: 'Panoramica dei contenuti che possiamo creare: reel, stories, foto, UGC, articoli o attivazioni su misura.',
    icon: Clapperboard,
  },
  {
    title: 'Asset e riferimenti',
    description: 'Informazioni operative, linee guida e dettagli utili per aprire un confronto in modo più serio.',
    icon: Download,
  },
];
...
export default function MediaKit() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!normalizedEmail || !normalizedCompany || !projectFocus || !normalizedBrief) {
      setSubmitError('Inserisci azienda, email lavorativa, focus del progetto e un breve brief per inviare la richiesta.');
      return;
    }
    ...
    try {
      ...
      setSubmitError(`Non siamo riusciti a registrare la richiesta. Puoi scriverci direttamente a ${CONTACTS.email}.`);
    } finally {
        ...
        <div className="mx-auto mt-8 max-w-5xl text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="mx-auto max-w-3xl text-xl font-normal leading-relaxed text-black/70">
              Questa pagina non serve a scaricare un PDF generico. Serve a darti un punto di accesso serio per capire
              chi siamo, come lavoriamo e che tipo di collaborazione possiamo costruire in modo credibile.
            </p>
            <div className="mt-8 inline-flex items-center gap-6 rounded-full border border-[var(--color-accent)]/20 bg-[var(--color-accent-soft)] px-8 py-3">
              <span className="text-xs font-bold uppercase tracking-widest text-[var(--color-accent)]">{BRAND_STATS.instagramFollowers} follower</span>
              <span className="h-4 w-px bg-[var(--color-accent)]/20"></span>
              <span className="text-xs font-bold uppercase tracking-widest text-[var(--color-accent)]">{BRAND_STATS.engagementRate} engagement</span>
              <span className="h-4 w-px bg-[var(--color-accent)]/20"></span>
              <span className="text-xs font-bold uppercase tracking-widest text-[var(--color-accent)]">{BRAND_STATS.monthlyReach} reach</span>
            </div>
          </motion.div>
        </div>
      </Section>
        ...
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            <div className="mb-10 rounded-[2rem] border border-black/5 bg-[var(--color-sand)] p-8 md:p-10">
              ...
              <p className="font-normal leading-relaxed text-black/70">
                Se il contatto è coerente con il progetto, condividiamo il materiale utile per valutare seriamente una
                possibile collaborazione: posizionamento, audience, formati, modalità di lavoro e riferimenti operativi.
              </p>
            </div>
          ...
          {!isSuccess ? (
            ...
            >
              ...
              <div className="mb-8">
                ...
                <p className="text-sm leading-relaxed text-zinc-500">
                  Lascia i dati essenziali del brand o del progetto. In questo modo possiamo capire subito se il
                  contatto è allineato e risponderti con materiali davvero utili.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Globe className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={18} />
                    <input
                      type="url"
                      id="website"
                      value={website}
                ...
                <div>
                  ...
                  <div className="relative">
                    <Target className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" size={18} />
                    <select
                      id="project-focus"
                      value={projectFocus}
                ...
                <p className="text-center text-xs leading-relaxed text-zinc-400">
                  Inviando questa richiesta accetti di essere ricontattato in merito a possibili collaborazioni e al
                  trattamento dei dati secondo la nostra{' '}
            ...
            >
              ...
              <h2 className="mb-4 text-2xl font-serif text-[var(--color-ink)]">Richiesta ricevuta</h2>
              <p className="font-light leading-relaxed text-[var(--color-accent-text)]">
                Grazie per l&apos;interesse. Ti ricontatteremo via email con il materiale più rilevante per capire se
                esistono le condizioni giuste per una collaborazione.
              </p>
            </motion.div>
          )}
        </div>
      ...
      <Section className="rounded-[3rem] bg-[var(--color-sand)] p-12 md:p-20">
        <div className="mb-12 text-center">
          ...
          <p className="mx-auto max-w-2xl font-normal leading-relaxed text-black/70">
            Non vogliamo aprire contatti generici. Preferiamo progetti allineati, in cui contenuto, brand e utilità per
            il pubblico possano stare davvero bene insieme.
          </p>
        </div>
      ...
      <Section className="rounded-[3rem] bg-[var(--color-ink)] p-12 text-white md:p-20">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mb-8 font-normal leading-relaxed text-white/80">
            Se preferisci partire da un contatto diretto invece che dal form, puoi scriverci via email o aprire una
            prima conversazione su WhatsApp. Il criterio resta lo stesso: allineamento, chiarezza e poi eventuale
            approfondimento.
          </p>

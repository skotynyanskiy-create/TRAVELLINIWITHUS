...
export default function AffiliateWidget({
  ...
  const configs = {
    insurance: {
      ...
      defaultDescription:
        'Non partire mai senza protezione. Abbiamo scelto Heymondo per la sua affidabilità e copertura completa.',
    ...
    booking: {
      ...
      defaultDescription:
        'Confronta prezzi, leggi le recensioni e prenota alloggi per ogni budget su Booking.com.',
    ...
    esim: {
      ...
      defaultDescription:
        'Evita il roaming costoso. Usa una eSIM Airalo per avere internet immediato ovunque nel mondo.',
    ...
    gear: {
      ...
      defaultDescription:
        'Scopri i prodotti che utilizziamo per i nostri viaggi e per creare i nostri contenuti.',
      defaultLink: '/risorse',
  ...
  return (
    <div className="my-12 bg-white rounded-[var(--radius-md)] border border-black/5 shadow-sm overflow-hidden flex flex-col md:flex-row">
      {image && (
      ...
      <div className="p-8 flex-1 flex flex-col justify-center">
        <div className="flex items-center gap-3 mb-4">
          {config.icon}
          <span className="uppercase tracking-widest text-[10px] font-bold text-[var(--color-accent)]">
      ...
      <div className="bg-[var(--color-sand)] p-8 flex flex-col justify-center items-center text-center sm:w-48 border-t md:border-t-0 md:border-l border-black/5">
        <div className="text-[var(--color-accent)] font-serif text-xl mb-1">
...
function LinkIconLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      href={href}
      className="text-[10px] uppercase tracking-widest font-bold text-black/40 hover:text-[var(--color-accent)] transition-colors"
    >
      {label}
    </a>
  );

...
export default function LatestArticles() {
  ...
  return (
    <Section title="Ultime storie" subtitle="Dal blog" className="bg-sand" ornament spacing="spacious">
      {loadingArticles ? (
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          {/* Articolo Principale */}
          <TiltCard className="lg:col-span-8" intensity={3}>
            <Link
                ...
                <div className="absolute top-6 left-6 flex items-center gap-2">
                  ...
                  {recentArticles[0].readTime && (
                    <span className="rounded-full bg-[var(--color-accent-warm)]/90 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-white backdrop-blur-md">
                      <Clock size={10} className="inline mr-1" />{recentArticles[0].readTime}
                    </span>
                  )}
                </div>
              </div>
            ...
            {recentArticles.slice(1, 3).map((article, idx) => (
              ...
              >
                ...
                <Link
                  ...
                  className="group relative flex flex-1 flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition-all duration-500 hover:shadow-xl h-full"
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <OptimizedImage
                      src={article.image}

  ...
  return (
    <section className="relative border-b border-[var(--color-border)] bg-[var(--color-sand)] py-24 text-[var(--color-ink)] md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <div className="group relative flex min-h-[460px] items-center overflow-hidden rounded-3xl border border-white/10 bg-[var(--color-ink-deep)] p-8 text-white shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-700 hover:border-[var(--color-accent-on-dark)]/40 md:p-16">
          <div ref={mapHostRef} className="absolute inset-0 z-0 overflow-hidden">
            {mapConsentGranted && mapInView ? (
              <Suspense fallback={<div className="h-full w-full bg-[var(--color-ink-deep)]" />}>
          ...
          <div className="relative z-10 grid w-full items-center gap-10 md:grid-cols-[1fr_auto] md:justify-between">
            <div className="min-w-0 max-w-2xl">
              <span className="inline-flex items-center gap-2 rounded-full border border-[var(--color-accent-on-dark)]/30 bg-[var(--color-accent)]/15 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.26em] text-[var(--color-accent-on-dark)] backdrop-blur-md">
                <Sparkles size={14} className="text-[var(--color-accent-on-dark)]" />
                Dove cercare
              </span>

              <h2 className="mt-5 font-serif text-3xl font-normal leading-tight text-white md:text-5xl lg:text-6xl">
                Trovali sulla mappa, <br />
                <span className="font-serif italic text-[var(--color-accent-on-dark)]">
                  prima di partire.
                </span>
              </h2>
              ...
              <div className="mt-8 flex flex-wrap items-center gap-6 border-t border-white/10 pt-6 text-xs font-medium text-[var(--color-sand)]/90">
                <div className="flex items-center gap-2">
                  <CheckCircle2 size={16} className="text-[var(--color-accent-on-dark)]" />
                  <span>Coordinate GPS esatte</span>
                </div>
                <span className="text-white/20">·</span>
                <div className="flex items-center gap-2">
                  <ShieldCheck size={16} className="text-[var(--color-accent-on-dark)]" />
                  <span>Posti provati di persona</span>
                </div>
                <span className="text-white/20">·</span>
                <div className="flex items-center gap-2">
                  <MapPin size={16} className="text-[var(--color-accent-on-dark)]" />
              ...
              {!mapConsentGranted && (
                <p className="mt-4 max-w-xl text-xs leading-relaxed text-[var(--color-sand)]/70">
                  ...
                  <button
                    type="button"
                    onClick={activateMap}
                    className="py-1 font-semibold text-[var(--color-accent-on-dark)] underline underline-offset-2 hover:text-white cursor-pointer"
                  >
                    Attivala
                  </button>
                </p>

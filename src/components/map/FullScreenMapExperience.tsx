        ...
        <div className="pointer-events-none absolute inset-x-4 top-6 bottom-16 z-40 flex flex-col gap-3 sm:inset-x-8 sm:bottom-6">
          <div className="flex shrink-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2 pointer-events-auto">
              ...
              <div className="flex items-center gap-1 overflow-x-auto rounded-full border border-stone-700 bg-stone-900/95 p-1.5 shadow-2xl backdrop-blur-2xl max-w-[calc(100vw-120px)] sm:max-w-none scrollbar-none">
                {FLY_PRESETS.map((preset) => (
                  <button
                    ...
                    className="shrink-0 rounded-full bg-stone-800 px-3 py-1 text-xs font-semibold text-stone-200 hover:bg-[var(--color-accent)] hover:text-[var(--color-ink)] transition-colors"
                  >
                    {preset.label}
                  </button>
                ))}
              ...
              >
                {locLoading ? (
                  <Loader2 size={14} className="animate-spin text-[var(--color-accent-on-dark)]" />
                ) : (
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-[var(--color-accent)]/50 bg-[var(--color-accent)] px-4 py-2 text-xs font-bold text-[var(--color-ink)] shadow-2xl transition-all hover:scale-105 hover:bg-white hover:text-stone-900"
              >
                <Shuffle size={14} />
              ...
              {listItems.length === 0 ? (
                <div className="flex flex-col items-center gap-3 py-8 text-center">
                  ...
                  <button
                    type="button"
                    onClick={resetView}
                    className="rounded-full bg-[var(--color-accent)] px-4 py-2 text-xs font-bold text-[var(--color-ink)] shadow-md transition-all hover:bg-white"
                  >
                    Reset vista
                  </button>
                </div>
                    ...
                    <button
                      ...
                      className={`w-full text-left rounded-xl p-3 border transition-all ${
                        selectedItem?.id === item.id
                          ? 'border-[var(--color-accent)] bg-[var(--color-accent)]/25 text-[var(--color-ink)] shadow-lg'
          ...
          {clusterEntities.groups.map((group) => (
            ...
            >
              <div className="flex min-h-11 min-w-11 cursor-pointer flex-col items-center justify-center gap-1">
                <div
                  className={`flex items-center justify-center rounded-full border-[1.5px] border-[var(--color-accent)] bg-black/85 font-bold tabular-nums text-white shadow-2xl transition-transform hover:scale-105 ${discSizeClasses(group.members.length).box} ${discSizeClasses(group.members.length).text}`}
                >
                ...
                >
                  {showName && (
                    <div
                      className={`relative mb-3.5 flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold shadow-2xl transition-all ${
                        isSelected
                          ? 'z-30 scale-110 border-2 border-white bg-[var(--color-accent)] text-[var(--color-ink)]'
                  ...
                  <div
                    className={`relative rounded-full border-2 border-white transition-transform group-hover:scale-125 ${
                      isSelected
                        ? 'h-3.5 w-3.5 bg-[var(--color-accent)] shadow-lg'
                        : 'h-2.5 w-2.5 bg-[var(--color-accent)] shadow-md'
                    }`}
                  >
                    {isSelected && (
                      <span className="absolute -inset-2 animate-pulse rounded-full bg-[var(--color-accent)] opacity-75 blur-sm" />
                    )}
                  </div>
                </div>
              </Marker>
              ...
              <div className="pointer-events-none mb-10 w-48 rounded-xl border border-white/20 bg-black/90 p-2.5 shadow-2xl backdrop-blur-md">
                {hoveredItem.cover ? (
                  ...
                  <div className="flex aspect-[4/3] w-full items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-stone-800 to-stone-900">
                    <MapPin size={24} className="text-[var(--color-accent)] opacity-60" />
                  </div>
                )}
        ...
        {selectedItem && (
          <div className="absolute bottom-16 left-4 right-14 z-50 mx-auto flex max-h-[min(68svh,calc(100%-4rem))] max-w-lg flex-col overflow-hidden rounded-[var(--radius-lg,24px)] border border-white/20 bg-black/90 p-6 text-white shadow-2xl backdrop-blur-2xl sm:bottom-10 sm:left-auto sm:right-20 sm:w-[460px] [@media(min-width:1024px)_and_(min-height:800px)]:max-h-[calc(100%-11rem)]">
            ...
            <div className="flex shrink-0 items-start justify-between">
              <div>
                <span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-accent)]/20 border border-[var(--color-accent)]/40 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--color-accent-text)]">
            ...
            <div className="mt-4 flex shrink-0 items-center justify-between border-t border-white/15 pt-4 text-xs font-semibold">
              ...
              <Link
                ...
                className="inline-flex items-center gap-1 rounded-full bg-[var(--color-accent)] px-4 py-2 text-xs font-bold text-[var(--color-ink)] shadow-md transition-all hover:bg-white hover:text-[var(--color-ink)]"
              >

...
function VideoPlane({
  ...
  useFrame(() => {
    ...
    // Il video è off-screen (opacity:0) e quindi non viene composto: il
    // requestVideoFrameCallback interno di VideoTexture non scatta e la texture
    // resterebbe nera. Forziamo l'upload del frame decodificato ogni frame.
  ...
  return (
    <group ref={groupRef}>
      <Float speed={1.3} rotationIntensity={0} floatIntensity={0.5} position={stage.position}>
        <Billboard>
          {/* Passe-partout scuro: bordo ink caldo dominante */}
          <mesh position={[0, 0.2, -0.05]}>
            <planeGeometry args={[3.7, 4.7]} />
            <meshBasicMaterial color="#1a1512" />
          </mesh>
          {/* Hairline accent terracotta: sottile anello visibile tra cornice e foto */}
          <mesh position={[0, 0.2, -0.04]}>
            <planeGeometry args={[3.18, 4.18]} />
            <meshBasicMaterial color="#c2410c" />
          </mesh>
          {/* Foto/reel: il soggetto principale */}
          <R3FErrorBoundary
            fallback={<NeutralPhoto position={[0, 0.2, 0.01]} width={3.1} height={4.1} />}
          >
            <Suspense
              fallback={<NeutralPhoto position={[0, 0.2, 0.01]} width={3.1} height={4.1} />}
            >
              {/* Poster sempre presente: cover reale del reel (statica, robusta).
                  Sul mondo attivo il VideoPlane qui sotto la copre col reel live. */}
              <CoverPhoto src={poster} position={[0, 0.2, 0.01]} width={3.1} height={4.1} />
            </Suspense>
          </R3FErrorBoundary>
          {/* v2 — reel in movimento: solo sul mondo attivo, un decoder alla volta.
              Sospeso sopra la cover, così finché il video carica si vede il poster. */}
          {showVideo && (
...
/** Riporta fuori dal canvas lo scroll: indice mondo attivo + barra + CTA finale.
 *  Aggiorna i ref direttamente nel useFrame (no re-render per frame). */
function ScrollReporter({
  ...
  useFrame((state, dt) => {
    ...
    // Scroll snapping fluido senza librerie esterne
    const el = scroll.el;
    if (el) {
      const delta = Math.abs(scroll.delta);
      if (delta > 0.0008) {
  ...
  return (
    ...
    >
      ...
      <EffectComposer>
        ...
        <Noise opacity={0.055} premultiply />
      </EffectComposer>
    </Canvas>
  );
}

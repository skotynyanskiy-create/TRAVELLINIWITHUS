...
| **C-LINT-2** | `set-state-in-effect` error in `useEffect(() => setStepIndex(0))`.                                                                                                  | `src/components/discovery/EsploraQuiz.tsx:76` (rimosso)                                                               |
| **C-LINT-3** | `<video>` senza `<track>` per captions (jsx-a11y/media-has-caption). Snapshot F1.8 lo dichiarava chiuso → regressione.                                              | `src/components/InstagramGrid.tsx:243`                                                                                |
...
| #          | Issue                                                                                                                                                                                                                                              | File                              |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| **C-UI-1** | `HomeTrustStrip` 4 KPI numeri serif + icone = pattern "overbuilt statistic strip" vietato da [DESIGN.md:48](../../DESIGN.md#L48). Direzione: convertire in 1 riga editoriale o spostare sotto fold.                                                | `HomeTrustStrip.tsx`              |
| **C-UI-2** | Sequenza `Hero → TrustStrip → PartnerSignal → DiscoveryFinder` = 3 sezioni "business-first" prima del primo segnale editoriale. Direzione: rimuovere/ridurre TrustStrip, spostare PartnerSignal dopo CoupleIntro, promuovere DiscoveryFinder a #2. | `Home.tsx:78-90`                  |
| **C-UI-3** | `HomeDiscoveryFinder` 4 card icon+title+arrow generiche tipo Linear/Notion. Direzione: 2 ingressi image-led basati sulle foto picks gia presenti sotto.                                                                                            | `HomeDiscoveryFinder.tsx:106-136` |

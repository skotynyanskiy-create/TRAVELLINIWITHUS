...
Tutti definiti in [`src/index.css:10-37`](../src/index.css#L10) dentro il blocco `@theme`. Uso tipico: classi Tailwind mappate (`bg-sand`, `text-ink`) oppure utility CSS var reali nel codice. Evitare placeholder di classi arbitrarie nei docs, perche Tailwind li puo interpretare come classi valide.
...
>
  {items.map((i) => (
    <motion.div key={i.id} variants={cardItem}>
      ...
    </motion.div>
  ))}
</motion.div>
...
| Colore brand, font family, radius brand-specific | **CSS var custom reali o classi tokenizzate**; evitare placeholder di utility arbitrarie                                                                                                |

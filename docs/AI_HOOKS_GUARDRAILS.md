...
| Claude `PreToolUse`    | `scripts/hooks/config_protection.py`    | blocking                      | Blocks edits that weaken guardrail configs (eslint, tsconfig, gitleaks, markdownlint) **and** edits to `firestore.rules` / `src/config/admin.ts`                                                                                                                                   | Override granulare: `HOOK_ALLOW_CONFIG_EDIT=firestore.rules` (o `=1` per tutti) |
...
| _(rimosso 2026-07-26)_ | ~~impeccable `hook.mjs`~~               | —                             | Mai eseguito: la skill `impeccable` non è installata su questa macchina e i tre file che la invocavano (`.codex/hooks.json`, `.cursor/hooks.json`, `.github/hooks/impeccable.json`) puntavano a `C:/Users/ccocu/…` o a `~/.claude/skills/`, entrambi inesistenti. File cancellati. | Non era in `settings.local.json`                                                |
| Test di regressione    | `scripts/hooks/test_hooks.py`           | verifica                      | 22 casi sui 4 hook: blocchi attesi, passaggi attesi, e i due buchi noti documentati (`git push origin +main`, falsi positivi su stringhe vietate dentro un `grep`)                                                                                                                 | `npm run test:hooks`                                                            |

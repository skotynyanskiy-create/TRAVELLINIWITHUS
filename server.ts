...
async function startServer() {
  ...
  const saveMonitoringEvent = async ({
    message,
    level,
    url,
    timestamp,
    ...
    const fields: Record<string, FirestoreValue> = {
      ...
      url: { stringValue: url || '' },
      createdAt: { timestampValue: timestamp || new Date().toISOString() },
      source: { stringValue: 'client-runtime' },
    };

    if (context && Object.keys(context).length > 0) {
      fields.context = {
        stringValue: JSON.stringify(context).slice(0, 12000),
      };
    }

    const response = await fetch(
      `https://firestore.googleapis.com/v1/projects/${firebaseConfig.projectId}/databases/${firebaseConfig.firestoreDatabaseId}/documents/monitoringEvents`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fields }),
    ...
    if (!response.ok) {
      throw new Error(
        `Monitoring event save failed: ${response.status} ${await response.text()}`
      );
    }
  ...
  app.post('/api/client-error', async (req, res) => {
    const { message, level, url, timestamp, context } = req.body as {
      message?: string;
      level?: 'error' | 'warning' | 'info';
      url?: string;
      timestamp?: string;
    ...
    if (!message?.trim()) {
      res.status(400).json({ error: 'Messaggio errore mancante.' });
      return;
    }

    const normalizedLevel =
      level === 'warning' || level === 'info' || level === 'error' ? level : 'error';

    try {
      const stored = await saveMonitoringEvent({
      ...
      if (!stored) {
        console.warn('Client monitoring event received without Firebase persistence.', {
          message,
          level: normalizedLevel,
          url,
        });
      ...
      console.error('Client monitoring save failed:', error);
      res.status(500).json({ error: 'Impossibile salvare il monitoraggio.' });
    }
  });
